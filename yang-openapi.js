...
(function() {
  ...
  var Yang, debug, definitions, discoverOperations, discoverPathParameter, discoverPaths, discoverTags, serializeJSchema, traverse, yaml, yang2jschema, yang2jsobj, yang2jsprop, yang2jstype;
    ...
    switch (schema.state.basetype) {
      ...
      case 'enumeration':
        return {
          type: 'string',
          format: schema.tag,
          enum: schema.enum.map(function(e) {
            return e.tag;
          })
        };
      default:
  ...
  yang2jsobj = function(schema) {
    var choices, js, property, ref, ref1, ref2, ref3, refs, required;
    if (schema == null) {
      return {};
    }
  ...
  yang2jschema = function(schema, item = false) {
    ...
    switch (schema.kind) {
      case 'leaf':
        return yang2jsprop(schema);
      case 'leaf-list':
        return {
          type: 'array',
          items: yang2jsprop(schema)
        };
      case 'list':
        if (!item) {
          return {
  ...
  discoverPathParameter = function(schema) {
    ...
    switch (false) {
      ...
      default:
        ...
        ref1 = yang2jsprop(schema.locate(param.name));
        for (k in ref1) {
          v = ref1[k];
          if (v != null) {
            param[k] = v;
...
// test if any subpaths have same param.name
  ...
  serializeJSchema = function(jschema) {
    var isProperty, k, o, ref1, ref2, v;
    if (jschema == null) {
      return;
    }
    isProperty = 'property' in jschema;
    o = {};
    for (k in jschema) {
      v = jschema[k];
      if (k !== 'property') {
  ...
  module.exports = require('./yang-openapi.yang').bind({
    transform: function(ctx, input) {
      ...
      switch (input['@choice']) {
        case 'swagger-file':
          throw ctx.error("swagger-file transform feature not yet supported!");
      }
      if (typeof debug === "function") {
        debug(`[transform] importing '${input.modules}'`);
      }
      modules = input.modules.map((name) => {
        return ctx.schema.constructor.import(name);
      }).filter(function(x) {
        return x != null;
      });
      if (!modules.length) {
    ...
    'grouping(specification)/serialize': function(ctx, input) {
      var ref1, spec;
      if (typeof debug === "function") {
